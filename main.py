...
def get_birthday(birthday, year, today):
    ...
    if today > year_date:
        if birthday_year[0] == "r":
            # 获取农历明年生日的月和日
            r_last_birthday = ZhDate((year + 1), r_mouth,
                                     r_day).to_datetime().date()
            birth_date = date((year + 1), r_last_birthday.month,
                              r_last_birthday.day)
        else:
...
# def get_weather(city, state_code):
#     try:
#         # get city_id using OpenWeatherMap API
#         api_key = '658cccf8c984b057a686fe441d455135'
...
#         tempn = round(data['main']['temp_min'] - 273.15, 1)
#         return weather, temp, tempn
#     except:
#         print(f"Failed to get weather data for {city}, {state_code}")
#         return None, None, None

def get_weather(city, state_code):
    url = f"https://api.weather.gov/gridpoints/LOT/72,80/forecast"
    headers = {"Accept": "application/geo+json"}
    response = requests.get(url, headers=headers)

    if response.status_code == 200:
        data = response.json()
        forecast = data["properties"]["periods"][1]["shortForecast"]
        temp = data["properties"]["periods"][1]["temperature"]
        temp = int((temp - 32) * 5 / 9)
        wind_speed = data["properties"]["periods"][1]["windSpeed"]
        return forecast, temp, wind_speed
    else:
        print(f"Failed to get weather data for {city}, {state_code}")
        return None, None, None
...
if __name__ == "__main__":
    ...
    # weather, max_temperature, min_temperature = get_weather(city, province)
    weather_data = get_weather(city, province)
    if weather_data is None:
        print("推送消息失败，请检查省份或城市是否正确")
        os.system("pause")
        sys.exit(1)
    else:
        weather, max_temperature, min_temperature = weather_data
    # 获取彩虹屁API
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, max_temperature,
                     min_temperature, pipi, lizhi, pop, tips,
                     note_en, note_ch, health_tip, lucky_)
    import time
